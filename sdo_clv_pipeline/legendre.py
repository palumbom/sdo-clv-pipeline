...
def gen_leg_vec(lmax, theta):
    ...
    theta : astropy.units.Quantity
        Colatitude in degrees (polar angle from the north pole, 90 - latitude;
        note ``SDOImage.lat`` stores latitude + 90 and must be converted).
    """
...
# derivative dP_l/dz; rho basis uses P_l(rho) directly (rho dimensionless).
# ---------------------------------------------------------------------------
...
def _bulk_vel_basis(theta_deg, phi_deg, rho, cos_B0, sin_B0, n_poly, scale, b):
    """Fill b[0:n_poly] with the bulk-velocity design row for one pixel.

    theta_deg is the true colatitude and phi_deg the longitude from the
    sub-observer meridian; lt and lp below are the line-of-sight projections of
    theta-hat and phi-hat for those angles and vanish at the sub-observer point.
    """
...
@njit(cache=True)
def bulk_vel_design(theta_deg, phi_deg, rho, cos_B0, sin_B0, n_poly, scale):
    ...
    identical normal-equations solve (im_arr @ im_arr.T). theta_deg is the true
    colatitude, phi_deg the longitude from the sub-observer meridian.
    ...
    for i in prange(n):
        b = np.empty(n_poly)
        _bulk_vel_basis(theta_deg[i], phi_deg[i], rho[i], cos_B0, sin_B0, n_poly, scale, b)
